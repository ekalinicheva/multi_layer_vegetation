...
def cloud_loader(plot_id, dataset, gt_raster, train, index_dict, args):
    """
    ...
    xymean = cloud_data[:, -4:-2] + args.plot_radius
    ...
    if "d" in args.input_feats:
        cloud_data[:, 5] = cloud_data[:, 5] / args.dist_max
    gt_points = cloud_data[:, args.n_input_feats].long()
    cloud_data = cloud_data.T

    if plot_id not in index_dict:
        xy = cloud_data[:2] * args.plot_radius + xymean.T + args.mean_dataset.reshape(2, 1)
        xy_min_cyl = cloud_data[-4:-2] + args.mean_dataset.reshape(2, 1)
        xy_round = torch.floor(xy * (1 / args.pixel_size)) / (1 / args.pixel_size)
        new_xy = ((xy_round - xy_min_cyl) / args.pixel_size)  # no matter what, we always clip by whole coordinates
        yx = new_xy[[1, 0], :].int()  # we swap x and y tp be able to pass to ij discrete coords
        yx[0] = args.diam_pix - 1 - yx[0]
        # index_dict[plot_id] = yx
    else:
        yx = index_dict[plot_id].clone()


    if train and args.data_augmentation:
        cloud_data = augment(cloud_data)


    # return regular_cylinders_norm, gt_tensor
    return cloud_data, gt, gt_points, yx
...
def cloud_collate(batch):
    ...
    """
    clouds, labels, labels_points, yx = list(zip(*batch))
    # if train:
    #     clouds = augment(clouds, labels)
    labels = torch.cat(labels, 0)
    labels_points = torch.cat(labels_points, 0)
    return clouds, labels, labels_points, yx
