...
for s in selected_placette_folders_final:
    ...
    if os.path.exists(ply_trees_placette) and os.path.exists(trees_csv) and int(pl_id)==15:
        ...
        full_placette_df['x'] = full_placette_df['x'] - x_min_plot
        full_placette_df['y'] = full_placette_df['y'] - y_min_plot
        ...
        for y in np.arange(0, y_max_plot- y_min_plot, pixel_size):
        # for y in np.arange(y_min_plot, y_max_plot, pixel_size):
            j = 0
            for x in np.arange(0, x_max_plot-x_min_plot, pixel_size):
        ...
        for tree_id in trees_ids:
            ...
            print(x_min, x_max)
            print(y_min, y_max)
            print(subset_tree['x_round'])
            print(subset_tree['y_round'])
        ...
        # matrix_plot_classes_canopy[(matrix_plot_classes_canopy != 4) & (matrix_plot_classes_canopy != 3) & (matrix_plot_classes_canopy != 7) & (
        #             matrix_plot_classes_canopy != 23) & (
        #         #             matrix_plot_classes_canopy != -1)] = 3    # chene + others
