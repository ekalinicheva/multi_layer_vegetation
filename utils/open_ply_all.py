...
def open_ply_all(args, selected_placettes=None, inference=False):
    print("open ply")
    ...
    for s in selected_placette_folders_final:
        ...
        ply_trees_placette = path_clouds + s + "/Pl_" + str(pl_id) + "_final_trees_with_none_legs.ply"
        ...
        if pl_id in placette_id_list or inference:
            ...
            data_ply_trees_placette = np.concatenate((data_ply_trees_placette, tree_height_class.reshape(-1, 1)), 1)
            ...
            if "d" in args.input_feats:
                if inference:
                    geo = [np.floor(data_final_df['x']).min(), args.pixel_size, 0, np.ceil(data_final_df['y'].max()), 0, -args.pixel_size]
                    H, W = int(np.ceil(data_final_df['y'].max()) - np.floor(data_final_df['y']).min()), int(np.ceil(data_final_df['x'].max()) - np.floor(data_final_df['x']).min())
            ...
            # print(np.concatenate((col_full, ['d'], ['tree_height_class']),0)[np.concatenate(([range(len(col_full)-1), [-2, -3, -1]]), 0)])
            # print(data_final_df[
            #         inversed_cols])
            #
            # print(data_ply_trees_placette[:10, 2])
            # print(data_ply_trees_placette_new[:10, 2])
