...
def reconstruct(pred_pointwise_all, cloud_all, stats_path, args, epoch_nb, cm, cm_2d):
    ...
    for b in range(len(all_plots)):
        ...
        new_ji = ((xy_round - (torch.floor(torch.min(xy_round, dim=1).values)).reshape(2, 1))/args.pixel_size).int()    # no matter what, we always clip by whole coordinates
        ij = new_ji[[1, 0], :]  # we swap j and i to be able to pass to ij discrete coords
        ij[0] = raster_size[1].int() - 1 - ij[0]   #we inverse the Y axis values, cause geo coords and raster indices do not increase in the same direction

        unique, index = torch.unique(ij, dim=1, return_inverse=True)
        unique = unique.long()
        ...
        # reconstructed_cloud_plot_pred = torch.cat((reconstructed_cloud_plot[:, 1:3], reconstructed_cloud_plot[:, 3:args.n_class-1].sum(1).reshape(-1, 1)), 1)
        ...
        if not args.inference:
            ...
            pred_name_tiff = stats_path + "Pl_" + str(pl_id.cpu().int().numpy()) + "_predicted_coverage.tif"
            pred_name_ply = stats_path + "Pl_" + str(pl_id.cpu().int().numpy()) + "_predicted_coverage.ply"
